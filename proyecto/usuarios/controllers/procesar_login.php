...
header('Location: ../views/iniciar_sesion.html');
exit;
...
function procesarLogin() {
    // Verificar token CSRF (relajado para testing)
    $token_csrf = $_POST['csrf_token'] ?? '';
    if (DESARROLLO) {
        // En desarrollo, aceptar cualquier token no vacío
        if (empty($token_csrf)) {
            logearActividad("Intento de login sin token CSRF desde IP: " . obtenerIPReal(), 'WARNING');
            redirigirConError(['Token de seguridad requerido. Recarga la página e inténtalo nuevamente.']);
        }
    } else {
...
function iniciarSesionUsuario($usuario, $email, $recordar = false) {
    ...
    $_SESSION['cliente_id'] = $usuario['idCliente'];  // Para compatibilidad con api_usuario_info.php
    ...
    if ($isAjax) {
        ...
        echo json_encode([
            'success' => true,
            'message' => "¡Bienvenido {$usuario['Nombre']}! Login exitoso.",
    ...
    // Redirección tradicional para formularios normales
