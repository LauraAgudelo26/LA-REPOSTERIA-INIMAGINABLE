...
define('DB_NAME', 'tienda_productos');
...
class DatabaseConnection {
    ...
    public function verificarLogin($email, $password) {
        try {
            ...
            $sql = "SELECT idCliente, Nombre, Apellido, Password_Hash, Estado, Fecha_Ultimo_Login 
                    FROM Cliente 
                    WHERE Correo_Electronico = ? AND (Estado = 'Activo' OR Estado IS NULL)";
            $stmt = $this->connection->prepare($sql);
            $stmt->execute([$email]);
            $usuario = $stmt->fetch();
            ...
            if ($usuario && verificarPassword($password, $usuario['Password_Hash'])) {
                ...
                $sql_update = "UPDATE Cliente SET Fecha_Ultimo_Login = NOW() WHERE idCliente = ?";
                $stmt_update = $this->connection->prepare($sql_update);
                $stmt_update->execute([$usuario['idCliente']]);
